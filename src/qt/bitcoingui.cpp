...
void BitcoinGUI::createActions()
{
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Digitalcoin command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::createToolBars()
{
    ...
    if(walletFrame)
    {
        ...
        // Add Digitalcoin logo on the right side
        QWidget* spacer = new QWidget();
        spacer->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
        toolbar->addWidget(spacer);

        QLabel *logoLabel = new QLabel();
        QPixmap logoPixmap(":/images/digitalcoin_logo_toolbar");
...
{
    QString strTitle = tr("Digitalcoin Core"); // default title
    ...
    // Append title to "Digitalcoin Core - "
    if (!msgType.isEmpty())
        strTitle += " - " + msgType;
