...
{
   unsigned int nProofOfWorkLimit = UintToArith256(ProofOfWorkLimit(algo)).GetCompact();
   ...
   int64_t nInterval = fInflationFixProtocol? (nTargetTimespanCurrent / nTargetSpacing) : (nTargetTimespanCurrent / (nTargetSpacing / 2));
    ...
    if (bnNew > UintToArith256(ProofOfWorkLimit(algo)))
        bnNew = UintToArith256(ProofOfWorkLimit(algo));
