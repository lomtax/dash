...
# digitalcoin_hash
RUN git clone https://github.com/digitalcoinpay/digitalcoin_hash
RUN cd digitalcoin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} digitalcoin
RUN useradd -u ${USER_ID} -g digitalcoin -s /bin/bash -m -d /digitalcoin digitalcoin
...
RUN mkdir /digitalcoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /digitalcoin-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /digitalcoin-src
