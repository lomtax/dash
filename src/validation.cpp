...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    // Check the header
    if (!CheckProofOfWork(block.GetPoWHash(), block.nBits, block.GetAlgo(), consensusParams))
...
void CheckForkWarningConditions()
{
    ...
    if (pindexBestForkTip || (pindexBestInvalid && pindexBestInvalid->nChainWork > chainActive.Tip()->nChainWork + (GetBlockWorkAdjusted(*chainActive.Tip()) * 6) && (chainActive.Height() > pindexBestInvalid->nHeight + 3 || pindexBestInvalid->nHeight > chainActive.Height() + 3)))
    {
...
void CheckForkWarningConditionsOnNewFork(CBlockIndex* pindexNewForkTip)
{
    ...
    if (pfork && (!pindexBestForkTip || (pindexBestForkTip && pindexNewForkTip->nHeight > pindexBestForkTip->nHeight)) &&
            pindexNewForkTip->nChainWork - pfork->nChainWork > (GetBlockWorkAdjusted(*pfork)* 20) &&
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block)
{
    ...
    pindexNew->nChainWork = (pindexNew->pprev ? pindexNew->pprev->nChainWork : 0) + GetBlockWorkAdjusted(*pindexNew);
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW)
{
    ...
    if (fCheckPOW && !CheckProofOfWork(block.GetPoWHash(), block.nBits, block.GetAlgo(), consensusParams))
...
bool static LoadBlockIndexDB(const CChainParams& chainparams)
{
    ...
    {
        CBlockIndex* pindex = item.second;
        pindex->nChainWork = (pindex->pprev ? pindex->pprev->nChainWork : 0) + GetBlockWorkAdjusted(*pindex);
