...
static CBlock BuildBlockTestCase() {
    ...
    assert(!mutated);
    while (!CheckProofOfWork(block.GetPoWHash(), block.nBits, block.GetAlgo(), Params().GetConsensus())) ++block.nNonce;
    return block;
}
...
BOOST_AUTO_TEST_CASE(EmptyBlockRoundTripTest)
{
    ...
    block.hashMerkleRoot = BlockMerkleRoot(block, &mutated);
    assert(!mutated);
    while (!CheckProofOfWork(block.GetHash(), block.nBits, block.GetAlgo(), Params().GetConsensus())) ++block.nNonce;
